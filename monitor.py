...
class Monitor(object):
    ...
    def start(self):
        ...
        pre_time = datetime.now(tz)
        try:
            while True:
                try:
                    cur_time = datetime.now(tz)
                    if is_run_crossing_midnight(pre_time, cur_time):
                        self.wx.send(heart_markdown(self.project, service, starttime), True)
                    for m in self.monitor_funcs:
                        m()
                    # self.kafka_monitor.monitor()
                    # self.yarn_app_monitor.monitor()
