...
CHECK_INTERVAL = 60                      # 监控周期（秒），默认 60 秒
...
class YarnSparkMonitor(object):
    ...
    def display_batch_summary(self, batches):
        ...
        for batch in batches:
            ...
            print(title.format(
                batch_id, input_size, get_duration(scheduling_delay), get_duration(processing_time), get_duration(total_delay), "{}/{}".format(complete, total)
            ))
    ...
    try:
        while True:
            ...
            if batches:
                ...
                log_error("获取批次信息失败，{} 秒后重试".format(CHECK_INTERVAL))
